...
namespace RPC
{
    ...
    template <int size>
    class rpc_scratch_buffer
    {
    public:
        rpc_scratch_buffer()
        {
            __buff = buff;
            __buff_len = sizeof(buff);
        }
        ~rpc_scratch_buffer() {}
    ...
    public:
        rpc() {}
        ~rpc() {}
        virtual bool read(uint8_t *buffer, size_t size, unsigned long timeout) = 0;
        virtual bool write(const uint8_t *buffer, size_t size, unsigned long timeout) = 0;
        bool same(uint8_t *data, size_t size);
        //virtual void begin() {}
        // virtual void end() {}
    ...
    class rpc_master : virtual public rpc
    {
    public:
        rpc_master() : rpc() {}
        ~rpc_master() {}
    ...
    class rpc_slave : virtual public rpc
    {
    public:
        rpc_slave() : rpc() {}
        ~rpc_slave() {}
...
/*
    class RS485MASTER : public rpc_master
    {
    };

    class RS485SLAVE : public rpc_slave
    {
    };

    class CANMASTER : public rpc_master
    {
    public:
        rpc_can_master(int message_id = 0x7FF, long bit_rate = 250E3) : rpc_master(), __message_id(message_id), __bit_rate(bit_rate) {}
        ~rpc_can_master() {}
        virtual void flush() override;
        virtual bool read(uint8_t *buff, size_t size, unsigned long timeout) override;
        virtual bool write(uint8_t *data, size_t size, unsigned long timeout) override;
        virtual void begin() override
        {
            CAN.begin(__bit_rate);
            CAN.filter(__message_id);
        }
    ...
    class CANSLAVE : public rpc_slave
    {
    public:
    ...
    class I2CMASTER : public rpc_master
    {
    public:
    ...
    class I2CSLAVE : public rpc_slave
    {
    public:
        ...
        virtual void begin() override
        {
            port.begin(__slave_addr);
            port.onReceive(onReceiveHandler);
            port.onRequest(onRequestHandler);
        }
    ...
    class SPIMASTER : public rpc_master
    {
    public:
    ...
    class SPISLAVE : public rpc_slave
    {

    }

    class SERIALMASTER : public rpc_master,
                         public Stream
    {
    public:
...
*/
    ...
    class RPCSERIAL : public rpc_master, public rpc_slave
    {
    public:
        RPCSERIAL(whatware w = RPC::HARDWARE, whatmode m = RPC::MASTER);
        void begin(Stream *S, uint32_t _timeout = 1000);
        void flush();
        int read();
        int available();
        void write(uint8_t c);
        bool read(uint8_t *buffer, size_t size, unsigned long timeout = 1000) override;
        bool write(const uint8_t *buffer, size_t size, unsigned long timeout = 1000) override;
        //void end();
        ~RPCSERIAL();
