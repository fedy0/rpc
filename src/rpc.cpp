...
// Written by Ifediora E C (fedy0)
// Date: 24 January, 2022
//
// Credits:
// Larry Bank, Kwabena W. Agyeman & Peter Dibia C.
...
uint8_t *RPC::__buff = NULL;
size_t RPC::__buff_len = 0;

rpc_callback_entry_t *RPC::__dict = NULL;
size_t RPC::__dict_len = 0;
size_t RPC::__dict_alloced = 0;
...
uint16_t rpc::__crc_16(uint8_t *data, size_t size)
{
    ...
    for (size_t i = 0; i < size; i++)
        crc = pgm_read_word_near(__crc_16_table + (((crc >> 8) ^ data[i]) & 0xff)) ^ (crc << 8);
...
bool rpc::same(uint8_t *data, size_t size)
{
    if (!size)
        return false;
...
bool rpc::stream_reader_setup(uint32_t queue_depth)
{
    ...
    if (!_stream_put_bytes(packet, sizeof(packet), 1000))
        return false;
    __tx_lfsr = 255;
    return true;
}
...
{
    uint8_t packet[8];
    if (!_stream_get_bytes(packet, sizeof(packet), 1000))
        return false;
    uint16_t magic = packet[0] | (packet[1] << 8);
    uint16_t crc = packet[6] | (packet[7] << 8);
    if ((magic != 0x542E) && (crc != __crc_16(packet, sizeof(packet) - 2)))
        return false;
    uint32_t size = unpack_unsigned_long(packet + 2);
    if (__buff_len < size)
        return false;
    if (!_stream_get_bytes(__buff, size, read_timeout))
        return false;
    if (!callback(__buff, size))
        return false;
    if (!_stream_put_bytes(&__tx_lfsr, sizeof(__tx_lfsr), 1000))
        return false;
...
bool rpc::stream_writer_setup()
{
    ...
    uint16_t magic = packet[0] | (packet[1] << 8);
    uint16_t crc = packet[6] | (packet[7] << 8);
    if ((magic != 0xEDF6) && (crc != __crc_16(packet, sizeof(packet) - 2)))
        return false;
...
bool rpc::stream_writer_loop(rpc_stream_writer_callback_t callback, unsigned long write_timeout)
{
    ...
    {
        if ((!_stream_get_bytes(packet, 1, 1000)) || (packet[0] != __rx_lfsr))
            return false;
    ...
    if (__credits > 0)
    {
        ...
        if ((!callback(&out_data, &out_data_len)) || (!out_data) || (!out_data_len))
            return false;
        _set_packet(packet, 0x542E, (uint8_t *)&out_data_len, sizeof(out_data_len));
        if (!_stream_put_bytes(packet, sizeof(packet), 1000))
            return false;
        if (!_stream_put_bytes(out_data, out_data_len, write_timeout))
            return false;
        __credits -= 1;
    }
...
bool rpc_master::__get_result(uint8_t **data, size_t *size, unsigned long timeout)
{
    ...
    while ((millis() - start) < timeout)
    {
        ...
        flush();
        write(__out_result_header_ack, sizeof(__out_result_header_ack), _put_short_timeout);
        if (_get_packet(_RESULT_HEADER_PACKET_MAGIC, __in_result_header_buf, sizeof(__in_result_header_buf), _get_short_timeout))
        {
            ...
            if (__buff_len < in_result_data_buf_len)
                return false;
            write(__out_result_data_ack, sizeof(__out_result_data_ack), _put_short_timeout);
                                      ...
                                      unsigned long send_timeout, unsigned long recv_timeout)
{
    return __put_command(_hash(name), NULL, 0, send_timeout)
               ? __get_result((uint8_t **)result_data, result_data_len, recv_timeout)
               : false;
}
                              ...
                              unsigned long send_timeout, unsigned long recv_timeout)
{
    ...
    bool result = __put_command(_hash(name), NULL, 0, send_timeout)
                      ? __get_result((uint8_t **)&result_pointer, &result_size, recv_timeout)
                      : false;
    if (return_false_if_received_data_is_zero)
        result = result && result_size;
    if (result)
        memcpy(result_data, result_pointer, min(result_data_len, result_size));
    else
...
bool rpc_slave::__get_command(uint32_t *command, uint8_t **data, size_t *size, unsigned long timeout)
{
    ...
    while ((millis() - start) < timeout)
    {
        ...
        if (_get_packet(_COMMAND_HEADER_PACKET_MAGIC, __in_command_header_buf, sizeof(__in_command_header_buf), _get_short_timeout))
        {
            ...
            if (__buff_len < in_command_data_buf_len)
                return false;
            write(__out_command_header_ack, sizeof(__out_command_header_ack), _put_short_timeout);
            if (_get_packet(_COMMAND_DATA_PACKET_MAGIC, __buff, in_command_data_buf_len, _get_long_timeout))
            {
                write(__out_command_data_ack, sizeof(__out_command_data_ack), _put_short_timeout);
                *command = cmd;
                *data = __buff + 2;
...
RPCSERIAL::RPCSERIAL(whatware w, whatmode m): rpc_master(), rpc_slave(), ware(w), mode(m)
{
   
}

void RPCSERIAL::begin(Stream* S, uint32_t _timeout)
{
    s = S;
    timeout = _timeout;
}

void RPCSERIAL::flush() 
{     
    if (ware == SOFTWARE)
    {
       //dynamic_cast<SoftwareSerial*>s->listen();
    }
    while (s->available())
    {
       s->read();
...
int RPCSERIAL::available()
{
    return s->available();
}

void RPCSERIAL::write(uint8_t c)
{
    s->write(c);
}

bool RPCSERIAL::read(uint8_t *buffer, size_t size, unsigned long timeout)
{ 
    bool ok;
    if (ware == SOFTWARE)
    {
    ...
    else 
    {
       ...
       if (mode == MASTER) ok = (s->readBytes((char *) buffer, size) == size) && (!rpc::same(buffer, size)); 
       else return s->readBytes((char *) buffer, size) == size;
    }
    
    if (!ok)
    {
       delay(_get_short_timeout);
    } 
    return ok;
}

bool RPCSERIAL::write(const uint8_t *buffer, size_t size, unsigned long timeout)
{
    return s->write((char *)buffer, size) == size;
}
